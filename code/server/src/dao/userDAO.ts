...
class UserDAO {
    ...
    deleteNonAdminUsers(): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            try {
        ...
        return new Promise<User>((resolve, reject) => {
            try {
                ...
                db.run(sql, params, function (err: Error | null) {
                    ...
                    db.get(selectSql, [username], (selectErr: Error | null, row: any) => {
                        if (selectErr) {
                            reject(selectErr);
                            return;
                        }
                        ...
                        const updatedUser: User = new User(row.username, row.name, row.surname, row.role, row.address, row.birthdate);
                        resolve(updatedUser);
                    });
                });
            } catch (error) {
