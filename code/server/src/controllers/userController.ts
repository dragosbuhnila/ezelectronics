...
class UserController {
    ...
    async deleteUser(user: User, username: string) /**:Promise<Boolean> */ {
        if((user.role  === 'Admin') && ((await this.dao.getUserByUsername(username)).role === 'Admin' && (await this.dao.getUserByUsername(username)).username != user.username)){
     ...
     */
    async deleteAll() { 
        return this.dao.deleteNonAdminUsers()
    }
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string) /**:Promise<User> */ {
        ...
        const temp = (await this.dao.getUserByUsername(username));
        if((user.username != username && user.role != 'Admin') || ((user.role ==='Admin' && temp.role === 'Admin') && (user.username != username))){
